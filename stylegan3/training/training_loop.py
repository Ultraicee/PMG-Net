...
from stylegan3.torch_utils.ops import grid_sample_gradfix
...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    gw = np.clip(7680 // training_set.image_shape[1], 7, 32)
    gh = np.clip(4320 // training_set.image_shape[0], 4, 32)
    ...
    # Load data.
    images, _ = zip(*[training_set[i] for i in grid_indices])
...
def training_loop(
        ...
        ada_interval=5,  # How often to perform ADA adjustment?
        ada_kimg=50,  # ADA adjustment speed, measured in how many kimg it takes for p to increase/decrease by one unit.
        total_kimg=2000,  # Total length of the training, measured in thousands of real images.
    ...
    if rank == 0:
        ...
        grid_size, images = setup_snapshot_image_grid(training_set=training_set)
        save_image_grid(images, os.path.join(run_dir, 'reals.png'), drange=[0, 100], grid_size=grid_size)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            phase_real_img = (phase_real_img.to(device).to(torch.float32) / 50.0 - 1).split(batch_gpu)  # [0, 100]->[-1, 1]
