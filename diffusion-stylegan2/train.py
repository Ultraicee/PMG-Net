...
def setup_training_loop_kwargs(
    ...
    cfg_specs = {
        'auto': dict(ref_gpus=-1, kimg=2000, mb=-1, mbstd=-1, fmaps=-1, lrate=-1, gamma=-1, ema=-1, ramp=0.05, map=2),
        # Populated dynamically based on resolution and GPU count.
        'stylegan2': dict(ref_gpus=1, kimg=2000, mb=32, mbstd=4, fmaps=1, lrate=0.002, gamma=10, ema=10, ramp=None,
                          map=2),  # Uses mixed-precision, unlike the original StyleGAN2.
        'paper256': dict(ref_gpus=4, kimg=25000, mb=64, mbstd=8, fmaps=0.5, lrate=0.0025, gamma=1, ema=20, ramp=None,
                         map=8),
        ...
        'paper1024': dict(ref_gpus=8, kimg=25000, mb=32, mbstd=4, fmaps=1, lrate=0.002, gamma=2, ema=10, ramp=None,
                          map=8),
        'cifar': dict(ref_gpus=4, kimg=100000, mb=64, mbstd=32, fmaps=1, lrate=0.0025, gamma=0.01, ema=500, ramp=0.05,
                      map=2),  # I changed ref_gpus from 2 to 4
        'stl10': dict(ref_gpus=4, kimg=50000, mb=64, mbstd=32, fmaps=1, lrate=0.0025, gamma=0.05, ema=500, ramp=0.05,
                      map=2),  # Added
    }
    ...
    args.G_kwargs = dnnlib.EasyDict(class_name='training.networks.Generator', z_dim=16, w_dim=16,
    ...
    args.G_kwargs.synthesis_kwargs.channel_base = args.D_kwargs.channel_base = 16 << 8
    args.G_kwargs.synthesis_kwargs.channel_max = args.D_kwargs.channel_max = 32
    args.G_kwargs.mapping_kwargs.num_layers = spec.map
    args.G_kwargs.synthesis_kwargs.num_fp16_res = args.D_kwargs.num_fp16_res = 2  # enable mixed-precision training
...
@click.option('--cfg', help='Base config [default: auto]',
              type=click.Choice(['auto', 'stylegan2', 'paper256', 'paper512', 'paper1024', 'cifar', 'stl10']))
@click.option('--gamma', help='Override R1 gamma', type=float)
@click.option('--kimg', help='Override training duration', type=int, metavar='INT')
@click.option('--batch', help='Override batch size', type=int, metavar='INT')
# Adaptive diffusion config.
@click.option('--beta_schedule', help='Forward diffusion beta schedule (we use linear always)', type=str,
              default='linear')
...
@click.option('--ts_dist', help='Diffusion t sampling way', type=click.Choice(['priority', 'uniform']),
              default='priority')
